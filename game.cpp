...
EX void uncoverMinesFull(cell *c2) {
  int mineradius = 
    bounded ? 3 :
...
EX int nearestPathPlayer(cell *c) {
...
EX void markAmbush(cell *c, manual_celllister& cl) {
  if(!cl.add(c)) return;
  forCellEx(c2, c) 
...
EX void terracottaAround(cell *c) {
  forCellEx(c2, c)
    terracotta(c2);
  }
